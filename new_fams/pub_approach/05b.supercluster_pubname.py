...
def supercluster(pub_idx):
    for i in pub_idx:
        ...
        for cl in clust_num["cluster"].drop_duplicates():
            super_cluster = "SELECT c.id, c.cluster, e.len, e.db, e.rna_type, d.description FROM eu_newfam_clstr c JOIN eu_newfam e ON e.id=c.id JOIN urs_description_con d ON d.id=c.id WHERE c.cluster = %i ORDER BY c.cluster" % (cl)
            ...
            super_cluster_pd = super_cluster_pd[["cluster", "id", "rep", "rna_type", "db", "len", "description"]]
